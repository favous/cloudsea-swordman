...
public class ReflectUtil {
	 ...
	 * @param list
	 */
	public static void getAllMethodWithParent(Class<?> claz, List<Method> list, boolean searchObject){
		if (!searchObject && claz == Object.class){
			return;
		}
		Method[] methods = claz.getDeclaredMethods();
		for (Method m : methods)
		...
		claz = claz.getSuperclass();
		if (claz != null)
			getAllMethodWithParent(claz, list, searchObject);
	}

	public static Method[] getAllMethodWithParent(Class<?> claz, boolean searchObject){
		List<Method> list = new ArrayList<Method>();
		getAllMethodWithParent(claz, list, searchObject);
		Method[] ms = new Method[list.size()];
